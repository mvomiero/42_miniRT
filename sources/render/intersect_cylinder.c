...
/*   Updated: 2023/07/05 17:03:07 by lde-ross         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		if (t1 > 0 && t1 < t2)
		{
			...
			if (check_cy(cylinder, hitpoint))
			{
				*t = t1;
				...
				{
					if (t1 < data->pix.t)
						data->pix.normal = vector_normalize(vector_substract(hitpoint, vector_add(cylinder->pos, vector_scale(axisDirection, vector_dot_product(vector_substract(hitpoint, cylinder->pos), axisDirection)))));
				}
		...
		if (t2 > 0 && t2 < t1)
		{
			...
			if (check_cy(cylinder, hitpoint))
			{
				*t = t2;
				...
				{
					if (t2 < data->pix.t)
						data->pix.normal = vector_normalize(vector_substract(hitpoint, vector_add(cylinder->pos, vector_scale(axisDirection, t2))));
				}
				return true;
			}
		}
