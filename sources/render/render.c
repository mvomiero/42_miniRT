...
/*   Updated: 2023/06/29 19:41:15 by lde-ross         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void shadePixelWithDiffuseColor(t_light *light, t_pixel *pixel)
{
	t_vect lightDirection;
	double dotProduct;

	// Calculate the direction from the light to the hitpoint
	lightDirection.x = light->pos.x - pixel->hitpoint.x;
	lightDirection.y = light->pos.y - pixel->hitpoint.y;
	lightDirection.z = light->pos.z - pixel->hitpoint.z;

	// Normalize the light direction vector
	double lightDistance = sqrt(lightDirection.x * lightDirection.x +
	...
	// Calculate the dot product between the light direction and the pixel normal
	dotProduct = lightDirection.x * pixel->normal.x +
				 lightDirection.y * pixel->normal.y +
				 lightDirection.z * pixel->normal.z;

	// Calculate the diffuse color
	if (dotProduct <= 0)
	{
		// No diffuse reflection, set color to black
		pixel->color.r = 0;
		pixel->color.g = 0;
		pixel->color.b = 0;
	}
	...
	{
		pixel->color.r = (unsigned char)(pixel->color.r * light->brightness * dotProduct);
		pixel->color.g = (unsigned char)(pixel->color.g * light->brightness * dotProduct);
		pixel->color.b = (unsigned char)(pixel->color.b * light->brightness * dotProduct);
	}
}
static void set_pixel_color(t_data *data, int x, int y, int color)
{
...
static void ray_tracer(t_data *data, int x, int y)
{
	...
	if (data->pix.t != INFINITY)
		shadePixelWithDiffuseColor(data->light, &(data->pix));
	else
