...
/*   Updated: 2023/07/04 16:42:07 by mvomiero         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_vect get_light_direction(t_light *light, t_pixel *pix)
{
    ...
    light_direction = vector_substract(light->pos, pix->hitpoint);  // Calculate the vector from the pixel's hitpoint to the light source
...
{
    int num_shadow_rays = 100;  // Number of shadow rays to cast
    double shadow_intensity = 0.0;  // Accumulator for shadow intensity
    double pixel_size = 1.0 / num_shadow_rays;  // Size of each pixel for sampling
    ...
    for (int i = 0; i < num_shadow_rays; i++)
    {
        ...
        t_vect shadow_direction = vector_substract(shadow_target, vector_add(pix->hitpoint, vector_scale(pix->normal, offset_x)));  // Calculate the direction of the shadow ray
    ...
    shadow_intensity /= num_shadow_rays;  // Calculate the average shadow intensity
